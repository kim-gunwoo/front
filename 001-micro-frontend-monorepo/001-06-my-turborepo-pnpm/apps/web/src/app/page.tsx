...
import { Card, Code, Button, Container } from '@repo/ui';
import { TurboIcon } from '@repo/icons';

function Gradient({ conic, className, small }: { small?: boolean; conic?: boolean; className?: string }): JSX.Element {
  return (
    <span
      className={[styles.gradient, conic ? styles.glowConic : undefined, small ? styles.gradientSmall : styles.gradientLarge, className]
        .filter(Boolean)
        .join(' ')}
    />
  );
...
export default function Page(): JSX.Element {
  return (
    <main className={styles.main}>
      <div className={styles.global_test}>global_test</div>
      <TurboIcon />
      <Image alt="" src={TurboIcon.src} height={20} width={20} style={{ pointerEvents: 'none' }} />
